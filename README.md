...
3. 同类模板只需确认一次。一次可混合上传 `应发小计`、`本月应发工资`、`本月工资`、`1` 等不同列名的文件，内置列名自动识别；弹窗优先显示待确认模板，在模板下拉框中分别选择 `本月工资`、`1` 对应的应发工资列即可一起合并。勾选“记住当前项目的对应关系”后，多套规则同时保留，下次同类模板直接复用，包括重启软件后；不同项目分别保存。规则仅在主动重置或重新设置时变化，表头结构变化、重名或对应列无法定位时仍需确认。只保存列头规则，不保存样例或工资数据。取消勾选则本次新设置不保存，已有规则不删除。
4. 列头列表不正确时，可展开“选择工作表与表头位置”重新读取。支持前 200 行内连续最多 6 行的表头，展示前 512 列的非空列名。
5. 需要调整或删除规则时，选择相应模板文件，点击主界面的“列头对应设置”。可修改对应列，或点击“恢复自动识别”，再“保存设置并关闭”；也可保存后继续合并。恢复自动识别只影响当前模板，不会删除其他列名模板的设置；直接取消弹窗不会保存本次调整。
