...
def _write_spec(
    ...
    *,
    ...
    if not QML_DIR.is_dir() or not (QML_DIR / "Main.qml").is_file():
        raise MacBuildError(f"缺少 Qt Quick 主界面资源：{QML_DIR}")
    if not QT_NOTICE.is_file() or not QT_HOOKS_DIR.is_dir():
        raise MacBuildError("缺少 Qt 打包许可说明或 PyInstaller 钩子")
    datas.extend(
        [
            (str(QML_DIR), "hr_toolkit/gui_qt/qml"),
            (str(QT_NOTICE), "third_party/qt"),
        ]
    )
